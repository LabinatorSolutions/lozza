...
for (var i=0; i < qp.length-6; i++) {
  var p = qp[i];
  docmd('ucinewgame');
  docmd('position ' + p[0]);
  docmd('id ' + p[2]);
  docmd('perft inner 0 ' + p[1]);
  var delta = lozza.stats.moves - lozza.stats.nodes;
  if (delta != 0)
    var warn = '************'
  else
    var warn = '';
  console.log(p[2],p[0],lozza.stats.moves,lozza.stats.nodes,delta,warn);
  n += lozza.stats.nodes;
}
...
console.log('nodes =',n,'sec =',myround(sec),'nps =',nps|0);
