...
var gWDLIndex      = -1;             // if -ve index from end of line as line.len + gWDLIndex.
var gUseScore      = true;
var gBatchSize     = 10000;
var gLearningRate  = 0.1;
var gResetAdagrad  = false;
var gOutFile       = 'gdtuner2.txt';
...
function readfile() {
  ...
  if (gVerbose)
    console.log('caching file',Vfileno);
  ...
  for (var i=0; i < lines.length; i++) {
    ...
    if (gWDLIndex < 0)
      var idx = parts.length + gWDLIndex;
    else
      var idx = gWDLIndex;

    var train = getprob(parts[idx]);
...
for (var i=0; i < gNumFiles; i++) {
  Vfileno = i;
  console.log(gPrefix + Vfileno + gSuffix + '.epd');
  readfile();
